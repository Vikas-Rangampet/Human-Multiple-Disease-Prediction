...
filename = 'C:\\Users\\Ranga\\Desktop\\Major project final 2023\\HealthCheck-Predicition-Main\\Models\\diabetes.pkl'

filename1 = 'C:\\Users\\Ranga\Desktop\\Major project final 2023\\HealthCheck-Predicition-Main\\Models\\cancer-model.pkl'
...
def ValuePredictor(final_features, size):
    loaded_model = joblib.load('models/heart_model')
    to_predict = np.array(final_features).reshape(1, size)
...
def predict_heart():
    ...
    if request.method == 'POST':
        try:
            final_features = request.form.to_dict()
            final_features = list(final_features.values())
            final_features = list(map(float, final_features))
            result = ValuePredictor(final_features, 11)
...
def liverprediction(final_features):
    ...
    to_predict = np.array(final_features).reshape(1, 10)
...
def predict_liver_disease():
    ...
    if request.method == 'POST':
        try:
            ...
            final_features = list(final_features.values())
            final_features = list(map(float, final_features))
            prediction = liverprediction(final_features)
        except:
            message = "Invalid Input"
            return render_template('liver.html', message=message)

    return render_template('liver_result.html', prediction=prediction)
...
@app.route("/k_predict", methods = ['POST', 'GET'])
def k_predictPage():
    
    try:
        if request.method == 'POST':
            int_features = request.form.to_dict()
            final_features = list(map(float, list(int_features.values())))
            pred = predict(final_features, int_features)
    except:
        message = "Please enter valid Data"
        return render_template("kidney.html", message = message)
    return render_template('k_predict.html', pred=pred)
